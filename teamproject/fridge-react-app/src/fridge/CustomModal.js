...
const CustomModal = ({ message, onClose, duration = 2500 }) => {
    ...
    // useEffect를 사용하여 message prop의 변경을 감지
    useEffect(() => {
        // 표시할 메시지가 있을 경우에만 타이머를 설정
        if (message) {
            const timer = setTimeout(() => {
                // duration 시간 후에 onClose 함수를 호출하여 모달을 닫음
            ...
            // 기존 타이머를 제거하여 메모리 누수를 방지용
            return () => clearTimeout(timer);
        }
    }, [message, onClose, duration]); // message, onClose, duration이 바뀔 때마다 이 효과를 재실행

    // 표시할 메시지가 없으면 아무것도 렌더링X
    if (!message) {
        return null;
    }
